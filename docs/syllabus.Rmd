...
glevel <- data.frame(Grade = c("A", "A-", "B+", "B", "B-", "C", "C-", "D"),
       `Class Tasks` = c(24, 24, 20, 18, 18, 15, 15, 5),
       `Case Studies` = c(13, 13, 8, 8, 6, 5, 5, 5),
       `Semester Project` = c("yes", "yes", "yes", "no", "no", "no", "no", "no"),
       Leadership = c("leader", "leader", "supporter", "supporter", "supporter", "wannabe", "wannabe", "-"), check.names = FALSE)

glevel %>%
  select(1:4) %>%
  knitr::kable(align = rep("c", 5), format = "html" ) %>%
  kable_styling(bootstrap_options = c("striped", "bordered", "hover"), full_width = F, position = "float_right") %>%
  group_rows("Leader", 1, 2) %>%
  group_rows("Supporter", 3, 5) %>%
  group_rows("Wannabe", 6, 7) %>%
  group_rows("None", 8, 8) %>%
  row_spec(0, bold = TRUE, color = "white", background = "black") %>%
  add_footnote(c("See [the tasklist](tasklist.html) for a description work related to leader, supporter, wannabe"), notation = "number")
...
* **A:** Complete at least `r glevel[1, "Class Tasks"]` class tasks, `r glevel[1, "Case Studies"]` case studies, the semester project, and the data science leader task 
* **A-:** Complete at least `r glevel[2, "Class Tasks"]` class tasks, `r glevel[2, "Case Studies"]` case studies, the semester project, and the data science leader task 
* **B+:** Complete at least `r glevel[3, "Class Tasks"]` class tasks, at least `r glevel[3, "Case Studies"]` case studies, at least 8 leadership completions, and all other semester deliverables
* **B:** Complete at least `r glevel[4, "Class Tasks"]` class tasks, at least `r glevel[4, "Case Studies"]` case studies, at least 8 leadership completions, and all semester deliverables excluding the semester project.
* **B-:** Complete at least `r glevel[5, "Class Tasks"]` class tasks, at least `r glevel[5, "Case Studies"]` case studies, at least 8 leadership completions, and the semester deliverables excluding the semester project.
* **C:** Complete at least `r glevel[6, "Class Tasks"]` class tasks, at least `r glevel[6, "Case Studies"]` case studies, at least 4 leadership completions, and the semester deliverables excluding the semester project.
* **C-:** Complete at least `r glevel[7, "Class Tasks"]` class tasks and at least `r glevel[7, "Case Studies"]` case studies
* **D:** Complete at least `r glevel[8, "Class Tasks"]` class tasks and at least `r glevel[8, "Case Studies"]` case studies
* **F:** Anything else
