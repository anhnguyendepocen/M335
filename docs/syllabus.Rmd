...
glevel <- tibble(Grade = c("A", "A-", "B+", "B", "B-", "C", "C-", "D"),
       `Class Tasks` = c(24, 22, 20, 18, 18, 15, 15, 5),
       `Case Studies` = c(13, 11, 8, 8, 6, 5, 5, 5),
       `On Time` = c("3/18/10", "3/18/10", "2/12/6", "2/12/6", "2/12/6", "1/8/4", "1/8/4", "0/0/0"),
...
glevel %>%
  select(1:5) %>%
  knitr::kable(align = rep("c", 6), format = "html" ) %>%
  ...
  add_footnote(c("See [the tasklist](tasklist.html) for a description of the 'On Time' column values"), notation = "number")
